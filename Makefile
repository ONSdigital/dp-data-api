...
.PHONY: audit
audit: ## Disabled audit as service is being deprecated
	exit

.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dp-data-api
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dp-data-api
...
debug: ## Used to run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dp-data-api
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dp-data-api
...
test-component: ## Runs component test suite
	go test -cover -coverpkg=github.com/ONSdigital/dp-data-api/... -component
